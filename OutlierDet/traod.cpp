...
void TRAOD::loadTRAODHurricane(QString file) {
    trajectoryData.readHurricaneFromFile(file);
}

void TRAOD::loadTRAODPortoTaxi(QString file) {
...
void TRAOD::processSegmentOrigOpt(CoarseSegment *coarseSegment, Trajectory *trajIn) {
    cl1->clear();
    cl2->clear();
    foreach (const Trajectory *traj, trajectoryData.listTrajectories) {
...
void TRAOD::createCoarseSegments() {
    foreach (Trajectory *traj, trajectoryData.listTrajectories) {
        qint64 startIndex = 0;
        qint64 length = 1;
...
double TRAOD::standardDeviation() {
    ...
    foreach (Trajectory *traj1, trajectoryData.listTrajectories) {
...
double TRAOD::standardDeviationAprox() {
    ...
    for (qint64 i = 0; i < trajectoryData.listTrajectories.size()/10; i++) {
        qint64 r = qrand() % (trajectoryData.listTrajectories.size()+1);
...
void TRAOD::densityForAllSegmenst() {
    ...
    foreach (Trajectory *traj1, trajectoryData.listTrajectories) {
        foreach (Segment *seg1, traj1->listSegments) {
            ...
            foreach (Trajectory *traj2, trajectoryData.listTrajectories) {
